...
def test_dataset_allows_fixture_only_validation():
    result = subprocess.run([
        sys.executable, 'scripts/validate_dataset_v1.py', '--schemas', 'schemas', '--sample', 'data/public_sample', '--check', 'all', '--allow-fixture-only'
...
def test_release_validation_fails_closed_for_fixture_only_sample():
    result = subprocess.run([
        sys.executable, 'scripts/validate_dataset_v1.py', '--schemas', 'schemas', '--sample', 'data/public_sample', '--check', 'all'
    ], cwd=ROOT, text=True, capture_output=True)
